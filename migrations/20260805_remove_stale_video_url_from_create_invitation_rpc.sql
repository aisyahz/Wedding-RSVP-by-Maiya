...
-- Fail safely if production does not have the audited 24-argument RPC.
-- CREATE OR REPLACE preserves the existing function ACL, so no REVOKE/GRANT
-- is needed (and no privilege can accidentally be broadened or removed).
DO $verify_signature$
DECLARE
    v_function_oid REGPROCEDURE := pg_catalog.to_regprocedure(
        'public.create_invitation_with_pin(text,text,text,date,time without time zone,text,text,text,text,text,text,text,text,text,text,timestamp with time zone,text,text,text,text,jsonb,integer,text,jsonb)'
    );
    v_argument_names TEXT[];
BEGIN
    IF v_function_oid IS NULL THEN
        RAISE EXCEPTION
            'Expected deployed create_invitation_with_pin signature was not found; migration aborted.';
    END IF;

    SELECT p.proargnames[1:24]
    INTO v_argument_names
    FROM pg_catalog.pg_proc AS p
    WHERE p.oid = v_function_oid;

    IF v_argument_names IS DISTINCT FROM ARRAY[
        'p_slug', 'p_bride_name', 'p_groom_name', 'p_wedding_date',
        'p_wedding_time', 'p_venue_name', 'p_venue_address',
        'p_google_maps_url', 'p_waze_url', 'p_whatsapp_contact',
        'p_wishlist_url', 'p_bank_name', 'p_bank_account_number',
        'p_bank_account_holder', 'p_qr_code_url', 'p_rsvp_closing_date',
        'p_video_key', 'p_video_file_name', 'p_status', 'p_custom_pin',
        'p_contacts', 'p_max_pax', 'p_dress_code_text',
        'p_dress_code_colors'
    ]::TEXT[] THEN
        RAISE EXCEPTION
            'Deployed create_invitation_with_pin argument names do not match the audited signature; migration aborted.';
    END IF;
END;
$verify_signature$;
...
BEGIN
    ...
    INSERT INTO public.invitations AS invitation (
        ...
        video_key, video_file_name, status
    ) VALUES (
        ...
        p_qr_code_url, p_rsvp_closing_date, p_video_key, p_video_file_name, p_status
...
-- PostgreSQL delivers transactional NOTIFY messages only after a successful commit.
