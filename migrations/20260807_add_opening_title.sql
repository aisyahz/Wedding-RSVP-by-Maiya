...
-- Replace the audited 24-argument creation RPC with a compatible
-- 25-argument version. The new final argument is optional, so existing
-- named-argument callers remain compatible after the migration.
...
CREATE FUNCTION public.create_invitation_with_pin(
    ...
    p_opening_title TEXT DEFAULT NULL
)
...
BEGIN
    ...
    INSERT INTO public.invitations AS invitation (
        slug, opening_title, bride_name, groom_name, wedding_date, wedding_time,
        venue_name, venue_address, google_maps_url, waze_url, whatsapp_contact,
        contacts, max_pax, dress_code_text, dress_code_colors, wishlist_url,
        bank_name, bank_account_number, bank_account_holder, qr_code_url,
        rsvp_closing_date, video_key, video_file_name, status
    ) VALUES (
        p_slug, NULLIF(pg_catalog.btrim(p_opening_title), ''), p_bride_name,
        p_groom_name, p_wedding_date, p_wedding_time, p_venue_name,
        p_venue_address, p_google_maps_url, p_waze_url, p_whatsapp_contact,
        COALESCE(p_contacts, '[]'::JSONB), p_max_pax,
        NULLIF(pg_catalog.btrim(p_dress_code_text), ''),
        COALESCE(p_dress_code_colors, '[]'::JSONB), p_wishlist_url,
        p_bank_name, p_bank_account_number, p_bank_account_holder, p_qr_code_url,
